...
public class SlingServerSideTestsBase extends SlingTestBase {
  ...
  public static final String SERVLET_NODE_PATH =
      "/test/sling/" + System.currentTimeMillis();

  /** Path used to request the Sling JUnit servlet */
  public static final String SLING_JUNIT_SERVLET_PATH =
      SERVLET_NODE_PATH + ".junit";
  ...
  /**
   * Before running tests, setup a node that gives access to the Sling JUnit
   * servlet, and check (with timeout) that the servlet is ready
   */
  public SlingServerSideTestsBase() {
    if (!servletNodeCreated) {
      final SlingClient slingClient = new SlingClient(
          getServerBaseUrl(), getServerUsername(), getServerPassword());
      try {
        slingClient.createNode(SERVLET_NODE_PATH, "sling:resourceType",
                               "sling/junit/testing");
    ...
    if (servletCheckFailed) {
      fail(SLING_JUNIT_SERVLET_PATH +
           " check failed previously, cannot run tests");
    }
