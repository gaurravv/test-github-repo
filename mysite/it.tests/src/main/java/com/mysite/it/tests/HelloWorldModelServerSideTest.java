 ...
 *
 *  <p>It relies on the <tt>ResourceResolverFactory</tt> to create test
 * resources and then adapt them to the class under test -
 * <tt>HelloWorldModel</tt>.</p>
 */
@RunWith(SlingAnnotationsTestRunner.class)
// ignore possible null pointer exceptions, deprecation warnings, loginAdmin
// usage, and generic exceptions
@SuppressWarnings({"squid:S2259", "squid:CallToDeprecatedMethod",
                   "AEM Rules:AEM-11", "squid:S00112"})
public class HelloWorldModelServerSideTest {

  @TestReference private ResourceResolverFactory rrf;

  @TestReference private SlingSettingsService settings;
  ...
  public void prepareData() throws Exception {
    new AdminResolverCallable() {
      ...
      protected void call0(ResourceResolver rr) throws Exception {
        rr.create(rr.getResource("/tmp"), "testResource",
  ...
  public void testHelloWorldModelServerSide() throws Exception {
    ...
    assertNotNull(
        "Expecting the ResourceResolverFactory to be injected by Sling test runner",
        rrf);
    assertNotNull(
        "Expecting the SlingSettingsService to be injected by Sling test runner",
        settings);
    ...
    new AdminResolverCallable() {
      ...
      protected void call0(ResourceResolver rr) throws Exception {
        ...
        assertNotNull("Expecting HelloWorldModel to be adapted from Resource",
                      hello);
